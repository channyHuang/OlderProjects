...
int main(int argc, char** argv)
{
	...
	std::string path = "E:/newData"; //insure this dir contains "pic" and "calrt" two subdirs, for saving raw pictures and camera parameters
	fusion.setDatapath(path);
	std::string videoName = path + "/VID_20160110_191705.mp4";
	...
	cv::Mat cameraMatrix = (Mat_<double>(3, 3) << 1410, 0, 800,
		0, 1410, 600,
		0, 0, 1);
	ifstream intriParam(picPath + "K.txt");
	if (!intriParam.eof()) {
		for (int x = 0; x < 3; x++) {
			for (int y = 0; y < 3; y++) {
				intriParam >> cameraMatrix.at<double>(x, y);
			}
		}
	}
	intriParam.close();
	...
	QDir dir((picPath + "/maskPic").c_str());
	...
	*/
	...
	for (int imageNum = 1; imageNum < numImg; imageNum++){
		...
		if (cv.count < imagesPerCV){
			...
			char str[100];
			sprintf(str, "%s/maskDepthmaps/%08d.jpg", picPath.c_str(), imageNum);
	...
	char str[100];
	for (int i = 0; i < numImg; i++) {
		sprintf(str, "%s/maskCalrt/%08d.txt", picPath.c_str(), i);
