...
Plot::Plot( QWidget *parent, int num ):
    QwtPlot( parent )
{
	plotnum = num;

    setTitle( QStringLiteral("访问人数动态图" ));

    QwtDateScaleDraw *scaleDraw = new DateScaleDraw( Qt::UTC );
    QwtDateScaleEngine *scaleEngine = new QwtDateScaleEngine( Qt::UTC );

	setAxisTitle(QwtPlot::xBottom, QStringLiteral("时间") );
    setAxisScaleDraw( QwtPlot::xBottom, scaleDraw );
    setAxisScaleEngine( QwtPlot::xBottom, scaleEngine );
    setAxisLabelRotation( QwtPlot::xBottom, -50.0 );
    setAxisLabelAlignment( QwtPlot::xBottom, Qt::AlignLeft | Qt::AlignBottom );

    setAxisTitle( QwtPlot::yLeft, QStringLiteral( "访问人数" ) );
...
void Plot::populate()
{
    ...
    for ( int i = 0; i < QuoteFactory::NumStocks; i++ )
    {
        ...
        curve->setSamples( QuoteFactory::samples2010( stock , plotnum) );
...
void Plot::exportPlot()
{
    ...
    renderer.exportTo( this, "chart.pdf" );
}
